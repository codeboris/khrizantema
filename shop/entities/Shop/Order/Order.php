 ...
 * @property int $cost
 * @property int $delivery_datetime
 * @property int $note
 * @property int $current_status
 * @property string $cancel_reason
 * @property CustomerData $customerData
 * @property RecipientData $recipientData
 *
...
class Order extends ActiveRecord
{
    ...
    public static function create($userId, CustomerData $customerData, RecipientData $recipientData, array $items, $cost, $delivery_datetime, $note): self
    {
        $order = new static();
        ...
        $order->items = $items;
        $order->cost = $cost;
        $order->delivery_datetime = (int)strtotime($delivery_datetime);
        $order->note = $note;
    ...
    public function edit(CustomerData $customerData, RecipientData $recipientData, $status, $delivery_datetime, $note): void
    {
        $this->customerData = $customerData;
        $this->recipientData = $recipientData;
        $this->addStatus($status);
        $this->delivery_datetime = (int)strtotime($delivery_datetime);
        $this->note = $note;
    ...
    public function attributeLabels()
    {
        return [
            ...
            //'delivery_method_name' => 'Текущий статус',
            //'deliveryData.index' => 'Индекс доставки',
            //'deliveryData.address' => 'Адрес доставки',
            'recipientData.name' => 'Имя получателя',
            'recipientData.phone' => 'Телефон получателя',
            'recipientData.address' => 'Адрес получателя',
            'cost' => 'Цена',
    ...
    public function afterFind(): void
    {
        ...
        $this->recipientData = new RecipientData(
            $this->getAttribute('recipient_phone'),
            $this->getAttribute('recipient_name'),
            $this->getAttribute('recipient_address')
        );
    ...
    public function beforeSave($insert): bool
    {
        ...
        $this->setAttribute('recipient_phone', $this->recipientData->phone);
        $this->setAttribute('recipient_name', $this->recipientData->name);
        $this->setAttribute('recipient_address', $this->recipientData->address);
